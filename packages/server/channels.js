export default function( app ) {
  ...
  app.publish(( /*data, hook*/ ) => {
    ...
    // To publish only for a specific event use
  ...
  // e.g. the publish the `users` service `created` event to the
  // `admins` channel
  // app.service('users').publish('created', () => app.channel('admins'));

  // With the userid and username organization from above you can easily
  // select involved users
  // app.service('messages').publish(() => {
  //   return [
  //     app.channel(`userIds/${data.createdBy}`),
  //     app.channel(`username/${data.recipientUsername}`)
  //   ];
  // });
}
